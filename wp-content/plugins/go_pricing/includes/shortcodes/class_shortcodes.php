...
class GW_GoPricing_Shortcodes {
	...
	public function pricing_table_sc( $atts, $content = null ) {
		...
		extract( shortcode_atts( array( 
			...
			'preview' => false,
			'data' => null,
			'text_replacements' => null //dimad@talknsave.net
		), $atts ) );
		...
		foreach ( (array)$pricing_table['col-data'] as $col_index => $col_data ) {
			...
			$col_wrap_header_html = sprintf( '<div id="go-pricing-table-' . $postid . '-column-wrapper-' . $col_index . '" %1$s%2$s%3$s>', 
			...
			$col_header_html = sprintf( 
				'<div%s id="go-pricing-table-' . $postid . '-column-' . $col_index . '"><div class="gw-go-col-inner"><div class="gw-go-col-inner-layer"></div><div class="gw-go-col-inner-layer-over"></div>', 
		...
		//BEGINNING dimad@talknsave.net
		//Replaces in $html. Format is "text => newtext; text => newtext"
		$text_replacements = array_map('trim', explode(';', $text_replacements));
		foreach ($text_replacements as $text_replacement) {
			if (trim($text_replacement) != "") {
				$text_replacement_fields = array_map('trim', explode(' =-+ ', $text_replacement));
				if (count($text_replacement_fields) == 2 && trim($text_replacement_fields[0]) != "" && trim($text_replacement_fields[1]) != "") {
					$html = str_replace($text_replacement_fields[0], $text_replacement_fields[1], $html);
				}
			}
		}
		//END
