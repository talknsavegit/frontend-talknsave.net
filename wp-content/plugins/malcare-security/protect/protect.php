...
if (!class_exists('MCProtect_V585')) :
...
class MCProtect_V585 {
	...
	public static function init($mode) {
		...
		if ($mode == MCProtect_V585::MODE_PREPEND) {
			$config_file = MCDATAPATH .  MCCONFKEY . '-' . 'mc.conf';
			$config = MCProtectUtils_V585::parseFile($config_file);

			if (empty($config['time']) || !($config['time'] > time() - (48*3600)) ||
					!isset($config['mc_conf_version']) ||
					(MCProtect_V585::CONF_VERSION !== $config['mc_conf_version'])) {
				return false;
			...
			$request = new MCProtectRequest_V585($request_ip_header, $req_config);
			$fw_config = array_key_exists('fw', $config) ? $config['fw'] : array();

			MCProtectFW_V585::getInstance($mode, $request, $fw_config, $brand_name)->init();
		} else {
			...
			MCProtectFW_V585::getInstance($mode, $request, $fw_config, $brand_name)->init();
			MCProtectLP_V585::getInstance($request, $lp_config, $brand_name)->init();
		}
	}
	...
	public static function uninstall() {
		...
		MCProtectIpstore_V585::uninstall();
		MCProtectFW_V585::uninstall();
		MCProtectLP_V585::uninstall();

		MCProtect_V585::removeWPPrepend();
		MCProtect_V585::removePHPPrepend();
		MCProtect_V585::removeMCData();
	...
	private static function removeWPPrepend() {
		...
		MCProtectUtils_V585::fileRemovePattern($fname, $pattern);
	}

	private static function removePHPPrepend() {
		MCProtect_V585::removeHtaccessPrepend();
		MCProtect_V585::removeUseriniPrepend();
	}

	private static function removeHtaccessPrepend() {
		$pattern = "/# MalCare WAF(.|\n)*# END MalCare WAF/i";

		MCProtectUtils_V585::fileRemovePattern(rtrim(ABSPATH, DIRECTORY_SEPARATOR) . "/.htaccess", $pattern, true);
	}

	private static function removeUseriniPrepend() {
		$pattern = "/; MalCare WAF(.|\n)*; END MalCare WAF/i";

		MCProtectUtils_V585::fileRemovePattern(rtrim(ABSPATH, DIRECTORY_SEPARATOR) . "/.user.ini", $pattern, true);
	}
	...
	private static function removeMCData() {
		...
		MCProtectUtils_V585::rrmdir($mc_data_dir);
	}
}
endif;