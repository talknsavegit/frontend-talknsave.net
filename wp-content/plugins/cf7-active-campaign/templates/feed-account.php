...
.vx_check_item input{
  ...
  ?>
  <div class="vx_wrap"> 
    <h2 class="vx_img_head"><img alt="<?php _e("Active Campaign Feeds", 'contact-form-activecamp-crm') ?>" title="<?php _e("Active Campaign Feeds", 'contact-form-activecamp-crm') ?>" src="<?php echo $this->get_base_url()?>images/activecamp-crm-logo.png?ver=1" /> <?php 
echo sprintf(__('Edit Active Campaign Feed # %d','contact-form-activecamp-crm') ,$fid); 
  if(empty($id) || $new_account_id != $id){
 ?> <a href="<?php echo $new_feed_link ?>" class="add-new-h2"><?php _e("Add New Feed",'contact-form-activecamp-crm'); ?></a> 
 <?php
}
 ?>
 <a href="<?php echo $feeds_link ?>" class="add-new-h2"><?php _e("Back to Feeds",'contact-form-activecamp-crm'); ?></a>
...
if(empty($feed_name)){
  ...
  ?>
  <div class="error" id="message" style="margin-top:20px;"><?php echo wpautop(sprintf(__("We are unable to login to Active Campaign with the provided API token. Please make sure they are valid in the %sSettings Page%s", 'contact-form-activecamp-crm'), "<a href='".$this->link_to_settings()."'>", "</a>")); ?></div>
  <?php
  // return;
  } 
  ?>
  ...
  <?php wp_nonce_field("vx_nonce") ?>
    <div class="vx_div">
      <div class="vx_head">
<div class="crm_head_div"> <?php _e('1. Enter feed name',  'contact-form-activecamp-crm' ); ?></div>
...
<div class="crm_clear"></div> 
  ...
  <div class="vx_row">
  <div class="vx_col1">
  <label for="vx_name" class="left_header"><?php _e("Feed Name", 'contact-form-activecamp-crm'); ?>
  ...
  <div class="vx_div">
        <div class="vx_head">
<div class="crm_head_div"> <?php _e('2. Select Active Campaign Account.', 'contact-form-activecamp-crm'); ?></div>
<div class="crm_btn_div" title="<?php _e('Expand / Collapse','contact-form-activecamp-crm') ?>"><i class="fa crm_toggle_btn vx_action_btn fa-minus"></i></div>
<div class="crm_clear"></div> 
  </div>
  ...
  <div class="vx_group">
    <div class="vx_row">
  <div class="vx_col1">
  <label for="vx_account" class="left_header"><?php _e("Active Campaign Account", 'contact-form-activecamp-crm'); ?>
  <?php $this->tooltip("vx_sel_object") ?>
 </label>
  </div>
  <div class="vx_col2">
  <select id="vx_account" class="crm_sel load_object" name="account" autocomplete="off">
  <option value=""><?php _e("Select a Active Campaign Account", 'contact-form-activecamp-crm'); ?></option>
  <?php
  ...
  <div class="clear"></div>
    ...
    <div class="vx_div">
        <div class="vx_head">
<div class="crm_head_div"> <?php _e('3. Select Contact Form.', 'contact-form-activecamp-crm'); ?></div>
<div class="crm_btn_div" title="<?php _e('Expand / Collapse','contact-form-activecamp-crm') ?>"><i class="fa crm_toggle_btn vx_action_btn fa-minus"></i></div>
<div class="crm_clear"></div> 
  </div>
  ...
  <div class="vx_group">
    <div class="vx_row">
  <div class="vx_col1">
  <label for="vx_form_id" class="left_header"><?php _e("Contact Form Form", 'contact-form-activecamp-crm'); ?>
  <?php $this->tooltip("vx_sel_form") ?>
 </label>
  </div>
  <div class="vx_col2">
  <select id="vx_form_id" class="crm_sel load_object" name="form_id" autocomplete="off">
  <option value=""><?php _e("Select a Form", 'contact-form-activecamp-crm'); ?></option>
  <?php
...
<?php
  ...
  ?>  
<div class="howto"><?php echo sprintf(esc_html__('If contact form is missing from list then install our %s , submit required form, it will add that missing form in this list.','contact-form-activecamp-crm'),'<a href="https://wordpress.org/plugins/contact-form-entries/" target="_blank">Contact Form Entries</a>')?></div>
<?php
      }
?>
  </div>
  <div class="clear"></div>
    ...
    <div id="crm_ajax_div_object" style="display:none; text-align: center; line-height: 100px;"><i> <?php _e('Loading, Please Wait...','contact-form-activecamp-crm'); ?></i></div>
...
//post validation
 ...
 if($(".vx_required").length){
   ...
   var value=""; 
    ...
    attach_error.append('<div class="entry_row vx_entry_error"><div class="vx_error"><i class="fa fa-warning"></i> <?php _e('This is a required field','contact-form-activecamp-crm') ?></div></div>');
    return false;
    }  
   })  
 }
  ...
  e.preventDefault(); 
    if(!confirm('<?php _e('Are you sure to remove ?','contact-form-activecamp-crm') ?>')){
      return;
  }
  ...
  if(field_name_select.length){
    ...
    if(field_name == '' || crm_fields[field_name] == ''){
     alert('<?php _e('Please Select Field Name','contact-form-activecamp-crm') ?>');
     return;   
    }  
...
if(type){ row.find('.vxc_field_option').hide(); }
  ...
  $(document).on('click','.vxc_tips',function(e){
    ...
    jQuery('#crm_note_fields').select2({ placeholder: '<?php _e('Select Field','contact-form-activecamp-crm') ?>'});
} 
...
vx_select();
  function vx_select(){
      $('#vx_add_fields_select').select2({ placeholder: '<?php _e('Select Field','contact-form-activecamp-crm') ?>'});
      add_note_sel();
  }
