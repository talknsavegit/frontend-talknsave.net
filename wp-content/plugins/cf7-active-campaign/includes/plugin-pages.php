...
if( !class_exists( 'vxcf_activecamp_pages' ) ) {
...
* @since       1.0.0
*/
class vxcf_activecamp_pages   extends vxcf_activecamp{
    public $ajax=false;
...
global $pagenow; 
 ...
 //add_filter("admin_menu", array($this, 'main_menu'), 29);
...
$page_title =__('CRM Plugins','contact-form-entries');
$capability = "vx_crmperks_view_plugins";
add_menu_page($page_title,$page_title,$capability,$menu_id,array( $this,'mapping_page'),'dashicons-cloud');
}   
 }
  /**
  ...
  */
  ...
  // Adding submenu if user has access
        $page_title =__('Active Campaign','contact-form-activecamp-crm');
        $menu_title =__('Active Campaign','contact-form-activecamp-crm');
...
add_submenu_page('vxcf_leads',$page_title,$menu_title,$capability,$this->id,array( $this,'mapping_page'));
 ...
 // add_submenu_page('cfx-form',$page_title,$page_title,$capability,$this->id,array( $this,'mapping_page'));
 //add_menu_page($page_title,$page_title,$capability,$this->id,array( $this,'mapping_page'));
...
public function add_bulk_send_crm_callcenter($list){ 
 $list['send_activecamp']=__('Send to Active Campaign','contact-form-activecamp-crm');
  return $list;  
}
public function bulk_send_crm_callcenter($msg,$action,$ids,$type){ 
    if($action == 'send_activecamp'){
...
$entry['__vx_id']=$entry['id'];
        ...
        if(empty($notice)){
            $notice='Nothing Sent to Active Campaign';
        }
...
$push=$this->push($entry_detail,$form,'add_note');
  ...
  ?>
  <div class="vx_row">
  <label><input type="checkbox" name="<?php echo $this->id ?>_send" value="yes"> <?php _e('Send to Active Campaign','contact-form-activecamp-crm') ?></label>
  </div>
  <?php  
}
    /**
  * Display custom notices
  * show activecamp response
  * 
  */
   ...
   $slug=$this->get_slug();
      if ( $file == $slug ) {
          $settings_link=$this->link_to_settings();
            array_unshift( $links, '<a href="' .$settings_link. '">' . __('Settings', 'contact-form-activecamp-crm') . '</a>' );
        }
        return $links;
   }
  /**
  ...
  */
  public  function mapping_page(){
      ...
      wp_enqueue_style('vxc-tooltip');
         $tabs=array('feeds'=>__('Active Campaign Feeds','contact-form-activecamp-crm') , 'logs'=>__('Active Campaign Log','contact-form-activecamp-crm') , 'accounts'=>__('Active Campaign Accounts','contact-form-activecamp-crm'), 'settings'=>__('Settings','contact-form-activecamp-crm'));
  ...
  $(this).text('<?php _e('Show Key','contact-form-activecamp-crm') ?>');  
  $(this).removeClass('vx_hidden');
  key.attr('type','password');  
  }else{
  $(this).text('<?php _e('Hide Key','contact-form-activecamp-crm') ?>');  
...
$class='';
 if($res){
       $msg=__('Account Deleted Successfully','contact-form-activecamp-crm');
  $class='updated';   
 }else{
       $msg=__('Error While Removing Account','contact-form-activecamp-crm');
  $class='error';      
 }
...
die();
  ...
  $msg=__('You do not have permissions to export logs','contact-form-activecamp-crm');
  ...
  $fields=array(); $field_titles=array("#",__('Status','contact-form-activecamp-crm'),__('Active Campaign ID','contact-form-activecamp-crm') ,__('Entry ID','contact-form-activecamp-crm'),__('Description','contact-form-activecamp-crm'),__('Time','contact-form-activecamp-crm'));
  ...
  check_admin_referer('vx_nonce','vx_nonce');
  if(!current_user_can($this->id."_edit_settings")){ 
  $msg=__('You do not have permissions to clear logs','contact-form-activecamp-crm');
  $this->display_msg('admin',$msg);
  return;   
  }
  $data=$this->get_data_object();
  $clear=$data->clear_logs();
       ...
       $msg=__('Error While Clearing Active Campaign Logs','contact-form-activecamp-crm');
      $level="error";
      if(!empty($clear)){
      $msg=__('Active Campaign Logs Cleared Successfully','contact-form-activecamp-crm');   
      $level="updated";
      }
  ...
  //
  self::$tooltips = array(
 'vx_feed_name' =>  __('Enter feed name of your choice.', 'contact-form-activecamp-crm'),
  'vx_sel_object' => __('Select the Object to Create when a Form is Submitted.', 'contact-form-activecamp-crm'),
   'vx_sel_account' =>__('Select the Active Campaign account you would like to export entries to.', 'contact-form-activecamp-crm'),
  'vx_sel_form' => __('Select the Contact Form you would like to integrate with Active Campaign. Contacts generated by this form will be automatically added to your Active Campaign account.', 'contact-form-activecamp-crm'),
  
  'vx_map_fields' => __('Associate your Active Campaign fields to the appropriate Contact Form fields.', 'contact-form-activecamp-crm'),
  
  'vx_optin_condition' =>__('When the opt-in condition is enabled, form submissions will only be exported to Active Campaign when the condition is met. When disabled all form submissions will be exported.', 'contact-form-activecamp-crm'),
  
  'vx_manual_export' => __('If you do not want all entries sent to Active Campaign, but only specific, approved entries, check this box. To manually send an entry to Active Campaign, go to Entries, choose the entry you would like to send to Active Campaign, and then click the "Send to Active Campaign" button.', 'contact-form-activecamp-crm'),
  
    'vx_entry_notes' => __('Enable this option if you want to synchronize Contact Form entry notes to Active Campaign Object notes. For example , when you add a note to a Contact Form entry, it will be added to the Active Campaign Object selected in the feed.', 'contact-form-activecamp-crm'),
    
      'vx_primary_key' => __('Which field should be used to update existing objects?', 'contact-form-activecamp-crm'),
      
  
  'vx_disable_logs'=>__('When an order is sent to Active Campaign we store that order information in the database and show it in the Active Campaign Log. Check this box if you do not want to save the exported order information in the logs.','contact-form-activecamp-crm'),
  
'vx_sources'=>__('Get Lead Sources list from activecamp.','contact-form-activecamp-crm'),
'vx_statuses'=>__('Get Lead Status list from activecamp.','contact-form-activecamp-crm'),
'vx_cats'=>__('Get Categories list from activecamp.','contact-form-activecamp-crm'),

'vx_source'=>__('Which Lead Source should be assigned to this object.','contact-form-activecamp-crm'),
'vx_status'=>__('Which Lead Status should be assigned to this object.','contact-form-activecamp-crm'),
'vx_cat'=>__('Which Category should be assigned to this object.','contact-form-activecamp-crm'),

'vx_source_check'=>__('If enabled, a Lead Source will be assigned to the Object','contact-form-activecamp-crm'),
'vx_status_check'=>__('If enabled, a Lead Status will be assigned to the Object','contact-form-activecamp-crm'),
'vx_cat_check'=>__('If enabled, a Category will be assigned to the Object','contact-form-activecamp-crm'),

'vx_assign_opportunity'=>__('Enable this option if you want to assign an opportunity this object.','contact-form-activecamp-crm'),
'vx_sel_opportunity'=>__('Select Opportunity feed. Opportunity created by this feed will be assigned to this object.','contact-form-activecamp-crm'),

'vx_assign_contact'=>__('Enable this option , if you want to assign a Contact to this object','contact-form-activecamp-crm'),
'vx_sel_contact'=>__('Select Contact feed. Contact created by this feed will be assigned to this object','contact-form-activecamp-crm'),

'vx_assign_organization'=>__('Enable this option , if you want to assign a Organization to this object','contact-form-activecamp-crm'),
'vx_sel_organization'=>__('Select Organization feed. Organization created by this feed will be assigned to this object','contact-form-activecamp-crm'),

   'vx_owner_check'=>__('Enable this option if you want to assign another object owner.','contact-form-activecamp-crm'),
   'vx_owners'=>__('Get Users list from Active Campaign','contact-form-activecamp-crm'),
   'vx_order_notes'=>__('Enable this option if you want to synchronize WooCommerce Order notes to Active Campaign Object notes. For example, when you add a note to a WooCommerce Order, it will be added to the Active Campaign Object selected in the feed.','contact-form-activecamp-crm'),
   'vx_sel_owner'=>__('Select a user as a owner of this object','contact-form-activecamp-crm'),
   
       'vx_entry_note'=>__('Check this option if you want to send more data as CRM entry note.', 'contact-form-activecamp-crm'),
   'vx_note_fields'=>__('Select fields which you want to send as a note', 'contact-form-activecamp-crm'),
   'vx_disable_note'=>__('Enable this option if you want to add note only for new CRM entry', 'contact-form-activecamp-crm')
...
<div class="table_head_i"><?php _e('Capsule', 'contact-form-activecamp-crm'); ?></div>  
<div class="vx_group">
...
echo $this->format_log_msg($log);
 ...
 $msg=__('You do not have permissions for this action','contact-form-activecamp-crm');
...
if( $this->has_feed($form_id)) { 
 $boxes['activecamp_crm']=array('title'=>'<i class="fa fa-plug"></i> '.__('Active Campaign', 'contact-form-activecamp-crm'),'callback'=>array($this,'meta_box_html'));  
} return $boxes; 
}
  ...
  case"refresh_users":
    ...
    $res['status']="ok";
     ...
     $res['error']=$users;   
        ...
        $data['crm_sel_user']=$this->gen_select($users,$owner_sel,__('Select User','contact-form-activecamp-crm'));
  $meta['users']=$users;  
  $res['data']=$data;   
      break;
...
if(isset($res['error'])){
    $res['status']='error';
    if(empty($res['error'])){
    $res['error']=__('Unknown Error','contact-form-activecamp-crm');
    }
}
  die(json_encode($res));    
  }
  ...
  */
  ...
  _e('You do not have permissions to access this page','contact-form-activecamp-crm');    
  return;
  }
  ...
  <p>
  <?php esc_html_e("Feed deleted.", 'contact-form-activecamp-crm') ?>
  </p>
  </div>
  <?php
  }
  ...
  <div class="updated fade" style="margin:10px 0;">
  <p>
  <?php esc_html_e("Feeds deleted.", 'contact-form-activecamp-crm') ?>
  </p>
  </div>
  <?php
  }
...
$count=$this->data->delete_log($log_ids);
  $this->screen_msg(sprintf(__('Successfully Deleted %d Item(s)','contact-form-activecamp-crm'),$count));  
    }
  ...
  if(!empty($entry) && is_array($entry)){ 
    ...
    $push=array('class'=>'error','msg'=>__('Entry Not Found','contact-form-activecamp-crm'));  
  }
...
$bulk_actions=array(""=>__('Bulk Action','contact-form-activecamp-crm'),"delete"=>__('Delete','contact-form-activecamp-crm'),
  'send_to_crm_bulk'=>__('Send to Active Campaign','contact-form-activecamp-crm'),'send_to_crm_bulk_force'=>__('Force Send to Active Campaign - Ignore filters','contact-form-activecamp-crm'));
$base_url=$this->get_base_url();
$objects=$this->get_objects(); 
$statuses=array("1"=>__("Created",'contact-form-activecamp-crm'),"2"=>__("Updated",'contact-form-activecamp-crm'),"error"=>__("Failed",'contact-form-activecamp-crm'),"4"=>__("Filtered",'contact-form-activecamp-crm'),"5"=>__("Deleted",'contact-form-activecamp-crm')); 
...
foreach($map as $field_k=>$field_v){
  ...
  $sel_fields=array(""=>__("Standard Field",'contact-form-activecamp-crm'),"value"=>__("Custom Value",'contact-form-activecamp-crm'));
...
public function groups($meta,$info){
 ...
 if(!empty($info['lists'])){ 
  ...
  <div class="vx_row">
   <div class="vx_col1">
  <label for="crm_sel_camp"><?php echo __('Select List(s)','contact-form-activecamp-crm') ?></label>
</div> <div class="vx_col2">
<?php
foreach($info['lists'] as $k=>$l){
$check= !empty($meta['lists'][$k]) ? 'checked="checked"' : '';    
?><div class="vx_check_item">
<input type="checkbox" <?php echo $check ?> id="meta_item_<?php echo esc_attr($k) ?>" name="meta[lists][<?php echo esc_attr($k) ?>]" value="1">
  ...
  */
  ...
  public function set_logging_supported($plugins) {
      $slug=$this->plugin_dir_name(); 
        $plugins[$slug] = __('Active Campaign','contact-form-activecamp-crm');
        return $plugins;
    }
  /**
...
$this->ajax=true;
  ...
  $msg=__("Please Choose Object",'contact-form-activecamp-crm');
  }else  if(empty($_REQUEST['form_id'])){
  $msg=__("Please Choose Form",'contact-form-activecamp-crm');
  }
  if($msg !=""){
  ?>
...
/*  $object=$this->post('object',$feed);
  ...
  $msg=__("Please Choose Account",'contact-form-activecamp-crm');
  }else  if(empty($_REQUEST['form_id'])){
  $msg=__("Please Choose Form",'contact-form-activecamp-crm');
  }
  if($msg !=""){
...
$field_options="<option>".__("Select Object",'contact-form-activecamp-crm')."</option>"; 
  if(is_array($objects)){
...
die();
  ...
  */
  ...
  if(!current_user_can($this->id.'_read_settings')){
  $msg_text=__('You do not have permissions to access this page','contact-form-activecamp-crm');   
  ...
  $this->uninstall();
  $uninstall_msg=sprintf(__("Contact Form Active Campaign Plugin has been successfully uninstalled. It can be re-activated from the %s plugins page %s.", 'contact-form-activecamp-crm'),"<a href='plugins.php'>","</a>");
$this->screen_msg($uninstall_msg);
  ...
  $meta=isset($_POST['meta']) ? $this->post('meta') : array();
   ...
   $msgs['submit']=array('class'=>'updated','msg'=>__('Settings Changed Successfully','contact-form-activecamp-crm'));
...
include_once(self::$path . "templates/settings.php");
 ...
 $offset=$this->time_offset();
  ...
  check_admin_referer("vx_nonce");
  if(!current_user_can($this->id."_edit_settings")){ 
  _e('You do not have permissions to save settings','contact-form-activecamp-crm');
  return;   
  }
  $msgs['submit']=array('class'=>'updated','msg'=>__('Settings Changed Successfully','contact-form-activecamp-crm'));
  $valid_email=true;
  ...
  if(!$valid_email){
      $msgs['submit']=array("class"=>"error","msg"=>__('Invalid Email(s)','contact-form-activecamp-crm'));
  }
...
//  $force_check=true;
  ...
  if($con_class != "updated" ){
      $msg=__('Connection to Active Campaign is NOT Working','contact-form-activecamp-crm');  
  }else{
     $msg=__('Connection to Active Campaign is Working','contact-form-activecamp-crm');   
  }
  $title=__('Test Connection: ','contact-form-activecamp-crm');
...
$feed= $this->data->get_feed($fid); 
  ...
  check_admin_referer("vx_nonce");
  if(!current_user_can($this->id.'_edit_feeds')){
  _e('You do not have permissions to edit/save feed','contact-form-activecamp-crm'); 
  return;
  }
  //
...
if(is_array($feed_update) && is_array($feed)){
  ...
  if($is_valid){
      ...
      $msgs['save']=array('class'=>'updated','msg'=>sprintf(__("Feed Updated. %sback to list%s", 'contact-form-activecamp-crm'), '<a href="'.$feed_link.'">', "</a>"));
  }
  else{
  $msgs['save']=array('class'=>'error','msg'=>__("Feed could not be updated. Please enter all required information below.", 'contact-form-activecamp-crm'));
...
if(is_array($tags)){
  ...
  $fields['__vx_id']=array('id'=>'__vx_id','label'=>__('Entry ID','contact-form-activecamp-crm')); 
$fields['_vx_form_id']=array('id'=>'_vx_form_id','label'=>__('Form ID','contact-form-activecamp-crm')); 
$fields['_vx_form_name']=array('id'=>'_vx_form_title','label'=>__('Form Title','contact-form-activecamp-crm')); 

  $this->fields=$fields=array('cf'=>array("title"=>__('Contact Form Fields','contact-form-activecamp-crm'),"fields"=>$fields));
...
if(is_array($fields_arr['fields'])){
  ...
  */
  ...
  $modules=array(""=>__("Select Object",'contact-form-activecamp-crm'));
  ...
  */
  ...
  $msg=__( 'Successfully Connected to Active Campaign','contact-form-activecamp-crm' );
     if(isset($info['_time'])){
  ...
  $info['msg']=!empty($info['error_msg']) ? $info['error_msg']: 'API Token is Not Valid'; 
  $info['class']="error"; 
  }
        }
    if($check){ 
