...
if( !class_exists( 'vx_crmperks_notice_vxcf_activecamp' )):

class vx_crmperks_notice_vxcf_activecamp extends vxcf_activecamp{
    
public $plugin_url="https://www.crmperks.com";
public $review_link='https://wordpress.org/support/plugin/cf7-active-campaign/reviews/?filter=5#new-post';
...
$msg =sprintf(__('Want to save contact form 7 submissions? Manage contact form 7 entries , View ActiveCampaign status of any entry and Send any entry to ActiveCampaign with free %sConatct Form Entries Plugin%s','crm-perks-forms'),'<a href="https://wordpress.org/plugins/contact-form-entries/" target="_blank">','</a>');
?>
...
<p><i class="fa fa-check"></i> View activecamp status(sent or not sent) of any entry.</p>
<p><i class="fa fa-check"></i> Easily Resend any entry to activecamp.</p>
...
//$install_time['review_closed']='';
    $time=current_time( 'timestamp' , 1 )-(3600*30);
 if(!empty($install_time) && is_array($install_time) && !empty($install_time['time']) && empty($install_time['review_closed'])){
  ...
  <p><?php echo sprintf(__( 'You\'ve been using Contact Form 7 ActiveCampaign Plugin for some time now; we hope you love it!.%s If you do, please %s leave us a %s rating on WordPress.org%s to help us spread the word and boost our motivation.','contact-form-entries'),'<br/>','<a href="'.$this->review_link.'" target="_blank" rel="noopener noreferrer">','&#9733;&#9733;&#9733;&#9733;&#9733;','</a>'); ?></p>
...
<p><i class="fa fa-check"></i> ActiveCampaign Contact tags.</p>
<p><i class="fa fa-check"></i> Add Contacts to multiple ActiveCampaign lists.</p>
<p><i class="fa fa-check"></i> ActiveCampaign Phone Number and All Custom fields.</p>
...
<p><b><a href="https://wordpress.org/plugins/support-x/" target="_blank">Support X - Wordpress Helpdesk</a></b> Shows user tickets from HelpScout, activecamp, FreshDesk, Desk.com and Teamwork in wordpress. Users can create new tickets and reply to old tickets from wordpress.</p>
</div>
<?php    
}
...
new vx_crmperks_notice_vxcf_activecamp();
endif;
