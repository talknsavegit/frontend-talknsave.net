...
* Author: CRM Perks.
* Text Domain: cf7-active-campaign
...
if( !class_exists( 'vxcf_activecamp' ) ):
...
class vxcf_activecamp {
 ...
 public  $url = "https://www.crmperks.com";
  ...
  public static $title='Contact Form Active Campaign Plugin';  
  public static $path = ''; 
  public static $slug = "";
...
//load_plugin_textdomain('cf7-active-campaign', FALSE,  $this->plugin_dir_name(). '/languages/' );
  ...
  $install=new vxcf_activecamp_install();
...
$this->push($entry,$form,'',false);    
  ...
  */
  ...
  if($type == "admin"){
     if($ver<4.2){
  ?>
    <div class="error vx_notice notice" data-id="<?php echo $id ?>"><p style="display: table"><span style="display: table-cell; width: 98%"><span class="dashicons dashicons-megaphone"></span> <b><?php _e('Contact Form Active Campaign Plugin','contact-form-activecamp-crm') ?>. </b><?php echo wp_kses_post($message);?> </span>
<span style="display: table-cell; padding-left: 10px; vertical-align: middle;"><a href="#" class="notice-dismiss" title="<?php _e('Dismiss Notice','contact-form-activecamp-crm') ?>">dismiss</a></span> </p></div>
  <?php
     }else{
  ?>
  <div class="error vx_notice notice below-h2 is-dismissible" data-id="<?php echo $id ?>"><p><span class="dashicons dashicons-megaphone"></span> <b><?php _e('Contact Form Active Campaign Plugin','contact-form-activecamp-crm') ?>. </b> <?php echo wp_kses_post($message);?> </p>
  </div>    
  <?php
     }
  }else{
...
*/
  ...
  $message = sprintf(__("%sContact Form Entries%s is required. %s it today!%s", 'contact-form-activecamp-crm'), "<a href='https://www.crmperks.com/'>", "</a>", "<a href='https://www.crmperks.com/'>", "</a>");   
  }else if($installed === 2){ // not active
  $message = sprintf(__('Contact Form Entries is installed but not active. %sActivate Contact Form Entries%s to use the Contact Form Active Campaign Plugin','contact-form-activecamp-crm'), '<strong><a href="'.wp_nonce_url(admin_url('plugins.php?action=activate&plugin=contact-form-entries-pro/contact-form-entries-pro.php'), 'activate-plugin_contact-form-entries-pro/contact-form-entries-pro.php').'">', '</a></strong>');  
  } else if($installed === 3){ // not supported
  $message = sprintf(__("A higher version of %sContact Form Entries%s is required. %sPurchase it today!%s", 'contact-form-activecamp-crm'), "<a href='https://www.crmperks.com/'>", "</a>", "<a href='https://www.crmperks.com/'>", "</a>");
  }  
...
return $objects;
  ...
  */
  ...
  $link="N/A"; $desc=__("Added to ",'contact-form-activecamp-crm');
  ...
  $desc.=$feed['object'];
  if($feed['status'] == 5){
   $desc=sprintf(__("%s Deleted",'contact-form-activecamp-crm'),$feed['object']);   
  }
  }else if($feed['status'] == 4){
      $desc=sprintf(__("%s Filtered",'contact-form-activecamp-crm'),$feed['object']); 
  }else{
  $desc= !empty($feed['error']) ? $feed['error'] : __("Unknown Error",'contact-form-activecamp-crm');
  }
  $title=__("Failed",'contact-form-activecamp-crm');   
  if($feed['status'] == 1){
  $title=__("Created",'contact-form-activecamp-crm');   
  }else if($feed['status'] == 2){
  $title=__("Updated",'contact-form-activecamp-crm');   
  }else if($feed['status'] == 4){
  $title=__("Filtered",'contact-form-activecamp-crm');   
  }else if($feed['status'] == 5){
  $title=__("Deleted",'contact-form-activecamp-crm');   
  }
...
do_action('plugin_status_'.$this->type,'activate');  
  ...
  * This string is sent to email and activecamp
...
public static function clean($var){
  ...
  */
  ...
  $status=0;
  if(class_exists('vxcf_activecamp_wp')){
      $status=1;
  }else if( file_exists(WP_PLUGIN_DIR.'/contact-form-activecamp-crm/contact-form-activecamp-crm.php')) {
  $status=2;
  } 
  return $status;
  }
 ...
 $offset = (int) get_option('gmt_offset');
  ...
  /**
  * Decrypts Values
  * @param array $info Active Campaign encrypted API info 
...
public function do_actions(){
  ...
  */
  ...
  $this->data=new vxcf_activecamp_data();
  return $this->data;
  }
...
$account=$this->post('account',$feed);
  ...
  if($this->post('id',$res)!=""){ 
      ...
      if($action == "Added"){
          if(empty($res['link'])){
  $msg=sprintf(__('Successfully Added to Active Campaign (%s) with ID # %s .', 'contact-form-activecamp-crm'),$feed['object'],$res['id']);
          }else{
  $msg=sprintf(__('Successfully Added to Active Campaign (%s) with ID # %s . View entry at %s', 'contact-form-activecamp-crm'),$feed['object'],$res['id'],$res['link']);
          }
  $screen_msg=__( 'Entry added in Active Campaign', 'contact-form-activecamp-crm');
      }else{
            if(empty($res['link'])){
  $msg=sprintf(__('Successfully Updated to Active Campaign (%s) with ID # %s . View entry at %s', 'contact-form-activecamp-crm'),$feed['object'],$res['id'],$res['link']);   
            }else{
  $msg=sprintf(__('Successfully Updated to Active Campaign (%s) with ID # %s .', 'contact-form-activecamp-crm'),$feed['object'],$res['id']);   
            }
          if($event == 'delete'){  
     $screen_msg=__( 'Entry deleted from Active Campaign', 'contact-form-activecamp-crm');
          }else{
     $screen_msg=__( 'Entry updated in Active Campaign', 'contact-form-activecamp-crm');
          }
          }
   
  
  }else if($this->post('status',$res) == 4){
  $screen_msg=$msg=__( 'Entry filtered', 'contact-form-activecamp-crm');    
  }else{
  $status=0; $screen_msg_class="error";
  $screen_msg=__('Errors when adding to Active Campaign. Entry not sent! Check the Entry Notes below for more details.' , 'contact-form-activecamp-crm' );
  if($log_id!=""){
      //message for  bulk actions in logs
  $screen_msg=__('Errors when adding to Active Campaign. Entry not sent' , 'contact-form-activecamp-crm' );    
  }
  $msg=sprintf(__('Error while creating %s', 'contact-form-activecamp-crm'),$feed['object']);
  if($this->post('error',$res)!=""){
      $error= is_array($res['error']) ? json_encode($res['error']) : $res['error'];
  $msg.=" ($error)";
  
  $_REQUEST['VXGActive CampaignError']=$msg; //front end form error for admin only
  }   
  if(!$is_admin){
      $info['msg']=$msg;
...
//  } 
  ...
  $log_link=' <a href="'.esc_url($log_url).'" class="vx_log_link" data-id="'.$insert_id.'">'.__('View Detail','contact-form-activecamp-crm')."</a>";
 $screen_msg.=$log_link;
    }
    if($post_comment){
...
//$this->add_note($entry["id"], $msg);
  ...
  */
  ...
  $subject="Error While Posting to Active Campaign";
  ...
  $detail["Form Id"]=$form['id'];
    ...
    $email_info=array("msg"=>$info['msg'],"title"=>__('Active Campaign','contact-form-activecamp-crm')." Error","info_title"=>"More Detail","info"=>$detail);
  ...
  /**
  * Get Objects from local options or from activecamp
  *     
...
$vxcf_activecamp=new vxcf_activecamp();
$vxcf_activecamp->instance();
$vx_cf['vxcf_activecamp']='vxcf_activecamp';
endif;
