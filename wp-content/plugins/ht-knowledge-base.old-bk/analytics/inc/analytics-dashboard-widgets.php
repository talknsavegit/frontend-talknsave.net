...
if( !class_exists( 'HKB_Analytics_Dashboard_Widgets' )) {
	class HKB_Analytics_Dashboard_Widgets {
		...
		function hkb_analytics_dashboard_widget_function(){
				...
				if ( $num_terms ) {
					$text = _n( '%s Article Category', '%s Article Categories', $num_terms, 'ht-knowledge-base' );
					$text = sprintf( $text, number_format_i18n( $num_terms ) );
					//@todo - restrict access to manage terms capability (doesn't exist in WordPress)
					printf( '<li class="%1$s-count hkb-analytics-dash-widget-categories"><a href="edit-tags.php?taxonomy=%1$s&post_type=ht_kb">%2$s</a></li>', $term, $text );
					
				}

				$total_article_views = HKB_Static_Stats::hkba_get_total_article_views();
				if ( $total_article_views ) {
					$text = __( '%s Total Article Views', 'ht-knowledge-base' );
					$text = sprintf( $text, number_format_i18n( $total_article_views ) );
					printf( '<li class="%1$s-views hkb-analytics-dash-widget-views"><a href="edit.php?post_type=ht_kb&page=hkb-analytics">%2$s</a></li>', 'feedback-stats', $text );
				...
				if ( false && $feedback_score ) {
					$text = __( '%s Satisfaction Rating', 'ht-knowledge-base' );
					$text = sprintf( $text, number_format_i18n( $feedback_score ) );
					printf( '<li class="%1$s-scores hkb-analytics-dash-widget-feedback"><a href="edit.php?post_type=ht_kb&page=hkb-analytics">%2$s</a></li>', 'feedback-stats', $text );
		...
		function hkb_analytics_dashboard_widget_admin_scripts() {
			...
			if( is_a( $screen, 'WP_Screen' ) && 'dashboard' ===  $screen->base ) {
				wp_enqueue_style( 'analytics-admin-dashboard-widgets-style', plugins_url('/css/analytics-dashboard-widgets.css', dirname( __FILE__ ) ) );
			}
		}
