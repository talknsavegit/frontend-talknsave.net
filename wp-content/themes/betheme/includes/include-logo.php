...
echo '<div class="logo mobileHeader'. esc_attr($logo_class) .'">';
	...
	if (isset($logo_options['link'])) {
		$logo_before = '<a id="logo" class="mobileHeader__logo" href="'. esc_url(get_home_url()) .'" title="'. esc_attr(get_bloginfo('name')) .'" data-height="'. intval($logo_height, 10) .'" data-padding="'. intval($logo_padding, 10) .'">';
		$logo_after = '</a>';
	} else {
...
echo '<a href="#" class="header_cart HeaderCustomCart popmake-17653 pum-trigger" style="cursor: pointer;"><span class="mobile-custom-cart"></span></a>';
echo '</div>';
