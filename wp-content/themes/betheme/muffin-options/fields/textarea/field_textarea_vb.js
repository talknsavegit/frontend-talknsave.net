  ...
  var MfnFieldTextarea = (function($) {
        ...
        tooltip:{
          ...
          toggle: () => {
            ...
            let shortcodeFocused = __scEditor.shortcode.focusedBrackets1,
              tooltip = $('.mfn-fr-show').find(' .mfn-form-row .editor-content .mfn-tooltip-sc-editor').clone().get(0),
          ...
          acceptButtonWatcher: (modal) => {
            ...
            $(acceptButton).one('click', function(e){
                try{
                  ...
                  var it = $('.mfn-form-row.content.mfn-fr-show').attr('data-element');
    ...
    let __cpTooltip = {
      ...
      toggle: _ => {
        ...
        __cpTooltip.domLocation = $('.mfn-fr-show .mfn-form-row .mfn-color-tooltip-picker');
        __cpTooltip.domLocation.toggleClass('focus');

        $('.mfn-fr-show .mfn-form-row.column.content').addClass('mfn-form-row-overlay');
        ...
        if(__cpTooltip.domLocation.hasClass('focus')){
          ...
          $('.mfn-fr-show .mfn-form-row .CodeMirror').addClass('preventClick');
      ...
      watchForOutsideClick: (e) => {
        ...
        if($(e.target).hasClass('mfn-icon-textcolor')){
          $('.mfn-fr-show .mfn-form-row .CodeMirror').removeClass('preventClick');
        ...
        updateView(__editor.instance.codemirror.getValue(), $(__cpTooltip.domLocation).closest('.mfn-element-fields-wrapper').attr('data-element'));
    ...
    function create() {
        ...
        try {
          if( $('.mfn-element-fields-wrapper.mfn-fr-show .html-editor .mfn-form-textarea').length ){
            __editor.domLocation = $('.mfn-element-fields-wrapper.mfn-fr-show .html-editor .mfn-form-textarea').get(0);
            $active = $('.sidebar-wrapper .mfn-element-fields-wrapper.mfn-fr-show');
            cmOn($active);
          }
    ...
    function cmOn($active){
      ...
      if($active.find('.mfn-form-row .preview-contentinput').siblings('.CodeMirror').length > 1){
        $active.find('.mfn-form-row .preview-contentinput').siblings('.CodeMirror').last().remove();
      }

      // inlineEditor

      var itemEl = $active.attr('data-element');
      ...
      if( !$iframeCont.hasClass('mfn-focused') ){
        ...
        inlineEditors[inlineIndex].setContent( __editor.instance.codemirror.getValue() );
      }
      ...
      if( !$iframeCont.hasClass('mfn-watchChanges') ){
        ...
        inlineEditors[inlineIndex].subscribe('editableInput', function(e, t){
            ...
            $('.sidebar-wrapper .mfn-fr-show .mfn-form-row .editor-content .preview-contentinput').val( $(t).html().replaceAll('&quot;', '') );
        });
      }

      // blur
      if( !$iframeCont.hasClass('mfn-blur-action') ){
        inlineEditors[inlineIndex].subscribe('blur', function(e, t){
          $iframeCont.addClass('mfn-blur-action');
          $iframeCont.removeClass('mfn-focused');
          if (!e.srcElement || ( e.srcElement && e.srcElement.innerText !== '%ME_PASTEBIN%' )) {
            re_render_content($(t).html(), itemEl);
          }
        });
      }
      ...
      __editor.instance.codemirror.on('blur', function(cMirror) {
          var container = $('.mfn-fr-show .mfn-form-row.content');
          var it = $('.mfn-fr-show').attr('data-element');
          $content.find('.'+it).addClass('loading disabled');
          if( !container.is(blurWatcher) && container.has(blurWatcher).length === 0 ){
            setTimeout(function() {
              let val = __editor.instance.codemirror.getValue()
              pending = true;
              $('.sidebar-wrapper .mfn-fr-show .mfn-form-row .preview-contentinput').val(val);
              re_render_content(val, it);
            }, 100);
          }
      });
      ...
      __editor.instance.codemirror.on('keyup',function(cMirror){
        $('.sidebar-wrapper .mfn-fr-show .mfn-form-row .editor-content .preview-contentinput').val( cMirror.getValue() );
        updateView();
      });
    ...
    function re_render_content(val, it) {
      $('.sidebar-wrapper .mfn-fr-show[data-element="'+it+'"] .mfn-form-row .editor-content .preview-contentinput').val(val);
        $.ajax({
    ...
    function updateView(val, it){
      ...
      //console.log('cm update '+val+' - '+it);

      val ? val = val : val = __editor.instance.codemirror.getValue();
      it ? it = it : it = $('.sidebar-wrapper .mfn-fr-show').attr('data-element');
    ...
    function destroy() {
        try {
          if( $('.mfn-fr-show .CodeMirror-wrap').length ){
